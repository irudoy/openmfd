...
void SingleGauge::update(bool instant)
{
  if (data->value != currentValue) {
    ...
    if (!shouldShowPeak) {
      if (data->scaler > 0) {
        Unicode::snprintfFloat(current_valueBuffer, CURRENT_VALUE_SIZE, "%3.2f", data->value / data->scaler);
      } else {
  ...
  if (data->peakValue != currentPeakValue) {
    ...
    if (shouldShowPeak) {
      if (data->scaler > 0) {
        Unicode::snprintfFloat(peak_valueBuffer, PEAK_VALUE_SIZE, "%3.2f", data->peakValue / data->scaler);
      } else {
