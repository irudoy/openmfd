...
void ILI9341_init(SPI_HandleTypeDef *spi)
{
  ...
  ILI9341_write_command(ILI9341_WRITE_MEMORY_CONTINUE);
  ILI9341_write_data(0x10);
  ILI9341_write_data(0x10);
  ILI9341_write_data(0x10);

  ILI9341_write_command(ILI9341_RGB_INTERFACE_CONTROL);
  ILI9341_write_data(0xC2); // ByPass_MODE = 1, RCM [1:0] = "10" (RGB Mode = DE mode)

  ILI9341_write_command(ILI9341_INTERFACE_CONTROL);
  ILI9341_write_data(0x01); // WEMODE = 0 => avoid white triangles
  ILI9341_write_data(0x00);
  ILI9341_write_data(0x02); // RM = 1 (Interface for RAM Access = RGB interface)

  ILI9341_write_command(ILI9341_MEMORY_ACCESS_CONTROL);
  // MY MX MV ML BGR MH 0 0
  // 0  1  1  0  1   0  0 0
  ILI9341_write_data(ILI9341_MADCTL_BGR);
...
void ILI9341_shift() {
  // No. of lines of the Frame Memory [not the display]
  ILI9341_write_command(ILI9341_VERTICAL_SCROLLING_DEFINITION);
  // TFA [15:8]
  ILI9341_write_data(0x00);
  // TFA [7:0]
  ILI9341_write_data(0x00);
   ...
   * on the display a
   */
  ILI9341_write_command(ILI9341_VERTICAL_SCROLLING_START_ADDRESS);
  // VSP [15:8]
  ILI9341_write_data(0x00);
  // VSP [7:0]
  ILI9341_write_data(shift);
